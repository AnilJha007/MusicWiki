...
class TagView<T> @JvmOverloads constructor(
    ...
    init {
        ...
        textColorId = styledAttrs.getColor(
            R.styleable.TagView_text_color,
            ContextCompat.getColor(context, R.color.white)
        )
        textFontSize = styledAttrs.getDimension(R.styleable.TagView_text_font_size, 1f).toInt()
        backgroundColorId = styledAttrs.getColor(
            R.styleable.TagView_background_color,
            ContextCompat.getColor(context, R.color.white)
        )
        ...
        leftDrawablePadding =
            styledAttrs.getDimension(R.styleable.TagView_left_drawable_padding, 1f).toInt()
        ...
        strokeColor = styledAttrs.getColor(
            R.styleable.TagView_stroke_color,
            ContextCompat.getColor(context, R.color.white)
        )
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        tags.forEach { child ->
            if (child.visibility != View.GONE) {
                ...
                val childWidth = child.measuredWidth
                lineHeight = max(lineHeight, child.measuredHeight + lp.verticalSpacing)
    ...
    private fun setTag(item: T, transform: DataTransform<T>) {
        val chips =
            (ContextCompat.getDrawable(context, R.drawable.chips) as GradientDrawable).also {
